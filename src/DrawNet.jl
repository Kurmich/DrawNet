...
#sequence to sequence variational autoencoder
function stroke_s2sVAE(model, data, seqlen, wkl; epsilon = 1e-6, istraining::Bool = true, dprob = 0)
  #model settings
  maxlen = maximum(seqlen) #maximum length of the input sequence
  M = Int((size(model[:output][1], 2)-3)/6) #number of mixtures
  (batchsize, V) = size(data[1])
...
function stroke_evaluatemodel(model, data, seqlens, wkl)
  rec_loss, kl_loss = 0, 0
  count = 0
  for i = 1:length(data)
    for j = 1:length(data[i])
      penstate_loss, offset_loss, cur_kl_loss = stroke_s2sVAE(model, map(a->convert(atype, a), data[i][j]), seqlens[i][j], wkl; istraining = false)
...
function stroke_minibatch(sketchpoints3D, numbatches, params)
